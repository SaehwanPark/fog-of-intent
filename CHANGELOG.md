...
- ADR-0004 Cargo Workspace Partitioning & `crates/foi-gui` Member Crate Extraction (Phase 6):
  - Extracted pure M11 presentation-only HTML5/CSS/SVG generator, client state machine, viewport layout, accessibility tree, transport session protocol, asset governance manifests, browser interaction flow evaluation, and bidirectional presentation parity engine into dedicated `crates/foi-gui` member crate.
  - Declared `foi-gui` as a workspace member in root `Cargo.toml` with path dependencies `foi-kernel = { path = "../foi-kernel" }`, `foi-lane = { path = "../foi-lane" }`, and `foi-protocol = { path = "../foi-protocol" }`.
  - Added `pub use foi_gui::*;` re-export facade in root `src/gui/mod.rs` for 100% backward compatibility with existing CLI, host, and MCP adapters.
  - Updated `scripts/check_repository.py` boundary file tracking for `crates/foi-gui/src/*.rs`.
  - Bumped workspace package versions to `0.1.238`.
  - Verified all 775+ workspace unit, binary, and doc tests pass with zero warnings.
