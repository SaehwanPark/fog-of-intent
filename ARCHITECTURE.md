...
- `complete_match.rs`: `CompleteMatchState` (integrated sequencing of the map, objective, vision, and structure state machines), `CompleteMatchAction` (`Rotate`/`PlaceWard`/`ContestObjectives`/`SiegeStructure`/`EvaluateTerminal`), `CompleteMatchPlan::execute` driving each action through its real subsystem transition (`m9-complete-match-v1`), and one combined FNV-1a hash committing every subsystem; fail-closed on empty, unterminated, post-conclusion, untracked-actor, and subsystem-rejected runs.
