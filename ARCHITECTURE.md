...
- `cost_profile.rs`: `OperationCounts` (exact transition/hash/projection/replay counters), `ScenarioCostProfile`, `ScalingProbe`, and `CostProfileReport` (`m9-cost-profile-v1`) with `profile_travel_scenario`, `profile_scaling_probe`, and `profile_catalog_batch` profiling the canonical travel catalog deterministically by operation counts at repository-edges-free wall-clock independence, including the [1, 8, 64, 512] step scaling ladder with linear transition growth and constant per-pass hash work. `catalog.rs` gains `execute_with_state` so profiling performs real terminal-state projections without sharing authoritative state.
