...
bounded player-facing Recall intent, and a bounded last-known threat report
implemented
...
- The M2 window, one bounded branch, allied proposal/coordination overlay, one
  ...
  bounded Recall intent, and last-known threat report remain bounded: no
  communication system, complete vision/belief model, variable pacing, gank
  response, resource restoration, external scenario serialization, branch tree,
  or broader debrief surface is implemented.
