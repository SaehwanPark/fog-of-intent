...
- `topology.rs`: `MapLocation` (`m9-map-topology-v1`) covering 15 discrete map locations (2 bases `MapLocation::ALLIED_BASE` and `MapLocation::OPPOSING_BASE`, both `Base(TeamSide)`, 9 lane sectors across `LaneId::Top`, `Mid`, `Bot`, 2 river zones `RiverSide::TopRiver`, `BotRiver`, and 2 jungle quadrants `JungleSide::TopJungle`, `BotJungle`).
...
- `state.rs`: `MatchMapState` with turn counter, multi-actor locations, deterministic FNV-1a state hashing, and `MatchMapObservation` projection with strict fog-of-war redaction (unseen rotating opponents are reported as `Unknown`). The interactive match host consumes this projection and preserves its two-valued certainty ladder — `Observed` or payload-free `Unknown` — in the actor-visible report: the projection never carries a stale position, and the `LastKnown` variant it once declared without ever emitting was removed so no writer can reintroduce one.
...
- `comeback_catalog.rs`: `ComebackScenarioDefinition`, `ComebackScenarioExecutionResult`, and `ComebackCatalog` (`m9-comeback-catalog-v1`) registering and executing 3 canonical benchmark scenarios (`scenario-teamfight-comeback-v1`, `scenario-desperation-all-in-v1`, `scenario-ahead-conservative-v1`).
...
- `pivotal_catalog.rs`: `PivotalScenarioDefinition`, `PivotalScenarioExecutionResult`, and `PivotalCatalog` (`m9-pivotal-catalog-v1`) registering and executing 3 canonical benchmark scenarios (`scenario-base-race-decisive-swing-v1`, `scenario-baron-throw-comeback-v1`, `scenario-stable-slow-burn-v1`).
...
- `decision_density_catalog.rs`: `DecisionDensityScenarioDefinition`, `DecisionDensityScenarioExecutionResult`, and `DecisionDensityCatalog` (`m9-decision-density-catalog-v1`) registering and executing 3 canonical benchmark scenarios (`scenario-routine-laning-absorption-v1`, `scenario-objective-spike-escalation-v1`, `scenario-decision-overload-v1`).
- `complete_match.rs`: `CompleteMatchState` (integrated sequencing of the map, objective, vision, and structure state machines), `CompleteMatchAction` (`Rotate`/`PlaceWard`/`ContestObjectives`/`SiegeStructure`/`EvaluateTerminal`), `CompleteMatchPlan::execute` driving each action through its real subsystem transition (`m9-complete-match-v2`), and one combined FNV-1a hash committing every subsystem (including the ward-id sequence and team membership); fail-closed on empty, unterminated, post-conclusion (including post-Nexus), untracked-actor, and subsystem-rejected runs. Contests and sieges resolve declared force against presence: `deliverable_force` bounds delivery by `FORCE_PER_PRESENT_ACTOR` per own actor in the target sector or `PRESENCE_REACH_BEATS` away (`MatchMapState::presence_within`), and `CompleteMatchState::force_declaration` states which sector a declaration targets so the host can refuse or explain it without inventing its own rule. `CommitStrength` (`light`/`committed`/`all-in`, priced at one actor's delivery, two, and the roster's via `MatchMapState::team_size`) supplies the player-facing spelling of a declaration; the host resolves it to an integer before the authority runs, so no separate force economy or legality token exists here.
- `complete_match_catalog.rs`: `CompleteMatchCatalog` (`m9-complete-match-catalog-v2`) registering 2 canonical complete matches (`scenario-complete-allied-snowball-v2` ending `NexusDemolished` at turn 14, `scenario-complete-comeback-concession-v2` ending `MatchConceded` at turn 34). Both plans rotate their actors into the sectors they siege, and the comeback plan fields three allied actors because two must hold the enemy base sector to reach the deep tiers. `all()` is the **benchmark set** — the plans the print-and-exit transcript runs, and it stays these two — while `find()` additionally resolves the interactive teaching plan `scenario-complete-onboarding-v1`, whose scripted line ends `NexusDemolished` at turn 6 (the turn that strikes it; the terminal evaluation beat follows) after one rotation walks its force into the enemy base. A teaching plan carries an initial position and no scripted actions, is never a benchmark member, and adds no rule, verb, or legality path.
...
- `population_validation_catalog.rs`: `PopulationScenarioDefinition`, `PopulationScenarioExecutionResult`, and `PopulationValidationCatalog` (`m9-population-validation-catalog-v1`) registering and executing 3 canonical benchmark scenarios (`scenario-diverse-engaged-population-v1`, `scenario-narrow-passive-population-v1`, `scenario-exempted-unused-mechanic-v1`).
...
- M3 has typed command contracts, bounded lane and match host fixtures, replay-validated
  ...
  a six-turn teaching match session (`m9-match-onboarding-v1`) selected like any other scenario and
  opened by an advisory briefing that reimplements no legality, sector names that round-trip from
  `observe` output back into `rotate`, `--run-dir` wiring, a matched-parent host branch projection,
