...
class FormatPolicyTests(unittest.TestCase):
  ...
  def test_rejects_hard_tabs_in_compatibility_fixtures(self) -> None:
    with tempfile.TemporaryDirectory() as directory:
      root = Path(directory)
      self.write_policy_files(root)
      ...
      self.assertTrue(any("tests/fixtures/legacy.txt" in error for error in errors))

  def test_rejects_missing_policy_files(self) -> None:
    with tempfile.TemporaryDirectory() as directory:
      errors: list[str] = []

      check_repository.check_format_policy(Path(directory), errors)

      self.assertIn(".editorconfig is missing", errors)
      self.assertIn("rustfmt.toml is missing", errors)

  def test_rejects_missing_policy_settings(self) -> None:
    with tempfile.TemporaryDirectory() as directory:
      root = Path(directory)
      (root / ".editorconfig").write_text(
        "root = true\n\n[*]\nindent_style = space\nindent_size = 2\n"
      ...
      self.assertTrue(any("tab_width" in error for error in errors))
      self.assertTrue(any("tab_spaces" in error for error in errors))

  def test_rejects_incorrect_policy_settings_tabs_and_four_space_python(self) -> None:
    with tempfile.TemporaryDirectory() as directory:
      root = Path(directory)
      (root / ".editorconfig").write_text(
        "root = true\n\n[*]\nindent_style = tab\nindent_size = 4\n"
      ...
      self.assertTrue(any("indent_style" in error for error in errors))
      self.assertTrue(any("tab_spaces" in error for error in errors))
      self.assertTrue(any("hard tab" in error for error in errors))
