...
fn advance_wave(pressure: WavePressure) -> Result<WavePressure, LaneExecutionError> {
  WavePressure::new(pressure.value() + 1).map_err(|_| LaneExecutionError::WaveOverflow { pressure })
}
...
fn apply_player_resources(
  ...
  let mana =
    before
      .mana()
      .subtract(deltas.mana_spent)
      .ok_or(LaneExecutionError::ManaExceedsAvailable {
        spent: deltas.mana_spent,
...
pub(super) fn resolve_lane_execution(
  ...
  for effect in state.delayed_effects.items().iter().flatten() {
    if let Some(remaining) = effect.delay().remaining_after(beats) {
      next_delayed_effects
        ...
        .map_err(|_| LaneTransitionError::Execution(LaneExecutionError::DelayedEffectOverflow))?;
    } else {
      delayed_effects_resolved.push(*effect);
      match effect.kind() {
        LaneDelayedEffectKind::SelfHealthRegen { amount } => {
          after_player_health = after_player_health.saturating_add(amount);
        }
