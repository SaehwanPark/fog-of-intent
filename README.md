...
| Rust package | `0.1.139`, edition 2024, Rust `1.96`, no dependencies, single package |
...
| Behavioral experiments and calibration | Versioned bounded experiment manifests and applicable version-catalog metadata, deterministic in-process batch, matched-observation, caller-supplied matched-scenario sample-set, selected-intent tally, machine-readable tally, fixed-fixture frequency, pure Markdown evidence, and caller-declared baseline comparison, plus injected cursor-resume evidence; decision persistence, population sampling, distributional/outcome/strategic metrics, independent build provenance, and calibration remain open — M6/M7 |
