...
| Rust package | `0.1.105`, edition 2024, Rust `1.96`, no dependencies, single package |
...
| Agent ecology and MCP | Three actor-visible deterministic scripted profiles with transparent policy-role IDs, matched-input evidence, bounded comparison/action-tally/candidate-breadth evidence, an opt-in reproducible seed bundle, library-only decision replay records, pure actor observation/action/draft/history/error/result DTO boundaries with bounded codecs, immutable actor-session lifecycle, actor-safe validation-error/repair hints, host-projected actor observations/history status, host-owned actor action submission/result/closure, and host-bound draft staging; broader ecology and MCP remain open — M4/M5 |
