...
| Rust package | `0.1.97`, edition 2024, Rust `1.96`, no dependencies, single package |
...
| Agent ecology and MCP | Three actor-visible deterministic scripted profiles with transparent policy-role IDs, matched-input evidence, bounded comparison/action-tally/candidate-breadth evidence, an opt-in reproducible seed bundle, library-only decision replay records, a pure actor observation/action DTO boundary with a bounded codec, immutable actor-session lifecycle, and actor-safe validation-error/repair hints; broader ecology and MCP remain open — M4/M5 |
