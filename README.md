...
| Rust package | `0.1.108`, edition 2024, Rust `1.96`, no dependencies, single package |
...
| Agent ecology and MCP | Three actor-visible deterministic scripted profiles with transparent policy-role IDs, matched-input evidence, bounded comparison/action-tally/candidate-breadth evidence, an opt-in reproducible seed bundle, library-only decision replay records, pure actor observation/action/commit/draft/draft-receipt/history/error/result/debrief DTO boundaries with bounded codecs, immutable actor-session lifecycle, actor-safe validation-error/repair hints, host-projected actor observations/history status/debrief, host-owned actor commit/action submission/result/closure, host-bound draft staging/receipts, and a bounded authorization/redaction matrix; broader ecology and MCP remain open — M4/M5 |
