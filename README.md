...
| Rust package | `0.1.98`, edition 2024, Rust `1.96`, no dependencies, single package |
...
| Agent ecology and MCP | Three actor-visible deterministic scripted profiles with transparent policy-role IDs, matched-input evidence, bounded comparison/action-tally/candidate-breadth evidence, an opt-in reproducible seed bundle, library-only decision replay records, a pure actor observation/action DTO boundary with a bounded codec, immutable actor-session lifecycle, actor-safe validation-error/repair hints, and read-only host action validation; broader ecology and MCP remain open — M4/M5 |
