...
| CLI reference experience | Complete — M3; TTY prompt, Tab completion, optional color, dynamic interactive scenario selection (`--select`), scenario catalog discovery (`--list-scenarios`), and `help`/`?` topics; pipes stay labeled plain text. Strategy, behavioral experiments, calibration proof, team benchmark, study synthesis, presentation export, browser flow evaluation, release checks, reproducibility bundle, release archive, and match scenario selections available |
