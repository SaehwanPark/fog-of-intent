...
| Rust package | `0.1.14`, edition 2024, Rust `1.96`, no dependencies, single package |
| Executable behavior | Prints `Hello, world!` |
| Deterministic kernel | M1 fixture/codec complete; M2 window, bounded variable-duration window, branch, coordination, objective, strategy fixtures, two-window wrapper, final debrief, bounded Recall intent, last-known threat projection, and conditional Withdraw response implemented internally |
| One-lane scenario | First diagnostic window, bounded variable-duration window, bounded branch, allied proposal/coordination, terminal objective, three strategy fixtures, two-window replay, final debrief, bounded Recall intent, last-known threat projection, and conditional Withdraw response implemented — full scenario not complete |
