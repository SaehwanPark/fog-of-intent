...
- [x] MCP JSON-RPC protocol surface parity across all 12 roadmap milestones in `src/mcp/tools.rs` and `src/mcp/server.rs` with 21 tools (`gui_presentation_render`, `alpha_release_checks_run`, `alpha_governance_audit`, `reproducibility_bundle_run`, `study_synthesis_run`, `team_scenarios_run`, `behavioral_experiments_run`, `replay_scenario`, `match_observe`, `match_plan_action`, `match_advance`, `match_debrief`, `observe`, `stage_draft`, `read_draft`, `clear_draft`, `commit_plan`, `advance_window`, `inspect_history`, `get_debrief`, `branch_scenario`), 3 prompts (`lane_decision_window`, `match_macro_turn`, `alpha_release_audit`), and 4 resources (`fog-of-intent://scenario/rules`, `fog-of-intent://session/state`, `fog-of-intent://release/readiness`, `fog-of-intent://presentation/html`).
...
- Integration tests in `tests/binary_run_dir.rs` and `scripts/verify_mcp_server.py` verify that `--scenario m12-alpha-release-checks-v1`, `--scenario m12-reproducibility-bundle-v1`, and all 21 MCP tools, 3 prompts, and 4 resources execute, render structured reports, maintain information boundaries, and exit cleanly with 80/80 passed assertions.
