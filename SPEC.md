...
- Run interactive gameplay validation across all canonical playable scenarios (M2 one-lane strategy playthroughs, M9 multi-lane tactical match runner).
...
- `src/mcp/` implements the Model Context Protocol (MCP) JSON-RPC 2.0 stdio server (`m5-mcp-stdio-adapter-v1`):
  ...
  - Exposes typed lane tools (`observe`, `stage_draft`, `read_draft`, `clear_draft`, `commit_plan`, `advance_window`, `inspect_history`, `get_debrief`, `branch_scenario`), multi-lane match tools (`match_observe`, `match_plan_action`, `match_advance`, `match_debrief`), and scenario verification tool (`replay_scenario`).
...
#### Delivered in the interactive multi-lane tactical match runner follow-up

- `--scenario m9-interactive-match-v1` provides a synchronous interactive multi-lane tactical match runner (canonical roster: 3 allied actors vs 1 opposing actor; see the roster and resolution limit in `ROADMAP.md` Phase 9):
...
- `m12-alpha-archive-v1` (`src/alpha/archive.rs`, `src/cli/alpha_archive.rs`, `src/command_loop.rs`, `src/main.rs`) implements official tagged research release archive manifest verification across 11 discrete categories (`SourceManifest`, `LockfileInventory`, `SchemaDefinitions`, `CatalogFixtures`, `ReplayEvidence`, `ModelCards`, `GovernanceManifests`, `CompatibilityMatrix`, `DataDictionary`, `DocumentationGuides`, `ReproducibilityBundle`), 16-hex FNV-1a hash integrity, combined archive signature verification, integer basis-point scoring ($[0..=10,000]$ bp), CLI scenario runner `--scenario m12-alpha-archive-v1`, MCP tool `alpha_release_archive_run`, and resource `fog-of-intent://release/archive`.
...
- [x] Extracted `crates/foi-map` member crate under Cargo workspace containing pure M9 multi-lane spatial map topology, 15-node map graph, 26 defensive structures hierarchy, neutral objective cycles, dynamic vision control, cross-map objective contest and tradeoff resolution, team composition archetypes, role-specific observations/actions/debriefs, comeback evaluation, pivotal-decision detection, decision density classification, cost profiling, population validation, expanded properties, and complete match simulation (`MatchMapState`, `MatchStructureState`, `MapTravelCatalog`, `ObjectiveScenarioCatalog`, `MatchScenarioCatalog`, `RoleScenarioCatalog`, `ComebackCatalog`, `PivotalCatalog`, `DecisionDensityCatalog`, `PopulationValidationCatalog`, `CompleteMatchCatalog`, `transition_travel`, `transition_objective_contest`, `transition_structure_siege`, `evaluate_comeback_opportunity`, `detect_pivotal_decisions`, `evaluate_decision_density`, `measure_validation_population`), re-exported cleanly via `crate::map` in root crate for 100% backward compatibility.
- [x] Extracted `crates/foi-agent` member crate under Cargo workspace containing pure M4/M6/M7/M8 behavioral policies, empirical calibrations, semantic trait dictionaries, regularized parametric policy fitting, diagnostic choice batteries, 8 canonical team speech acts, dialogue sessions, caller reputation tracking, transmission channel physics, designated shot-calling heuristics, decentralized consensus arbitration, private submission collection, simultaneous multi-agent resolution, decoupled coordination vs execution attribution, causal communication and leadership debriefs, strategic disagreement proofs, and canonical scenario benchmark catalogs (`ScriptedAgent`, `ScriptedAgentProfile`, `ScriptedAgentBatchRunner`, `ScriptedAgentOperationalLog`, `DiagnosticChoiceCatalog`, `SemanticProfileVocabulary`, `ParametricPolicyFitter`, `TeamSpeechAct`, `TeamDialogueSession`, `TeamCommunicationChannel`, `TeamTrustEvaluator`, `TeamLeadershipEvaluator`, `TeamSimultaneousResolver`, `TeamAttributionEvaluator`, `TeamScenarioCatalog`), re-exported cleanly via `crate::agent` in root crate for 100% backward compatibility.
- [x] Extracted `crates/foi-protocol` member crate under Cargo workspace containing pure model-agnostic actor-visible observation, action, metadata, lifecycle, result, commit, draft, error, history, intent, message, observation, replay, transcript DTOs, and MCP wire codecs (`ActorActionDto`, `ActorActionResultDto`, `ActorCommitDto`, `ActorCommitResultDto`, `ActorDebriefDto`, `ActorDraftDto`, `ActorDraftReceiptDto`, `ActorDraftStatusDto`, `ActorDraftClearDto`, `ActorProtocolError`, `ActorProtocolErrorCode`, `ActorProtocolRepairHint`, `ActorHistoryDto`, `ActorProtocolIntent`, `ActorMessageDto`, `ActorCommunicationAbusePopulationReport`, `ActorObservationDto`, `ActorReplayDto`, `ActorReplayRecordDto`, `ActorReplayDebriefRecordDto`, `ActorTranscriptDto`, `ActorToolCapability`), re-exported cleanly via `crate::protocol` in root crate for 100% backward compatibility.
- [x] Extracted `crates/foi-study` member crate under Cargo workspace containing pure M10 human usability, accessibility alpha study protocol definitions, participant cohort criteria, completion tracking, finding taxonomy, severity ranking, issue-linked disposition tracking, basis-point cohort evaluation, dimension-level assessments, interaction auditing, informal check protocols, remediation plan verification, participant sampling limits, untested population disclosures, alpha evidence synthesis, and readiness disposition gates (`StudyProtocolCatalog`, `ParticipantDimensionAssessment`, `EmpiricalTrialsCatalog`, `evaluate_empirical_trials`, `evaluate_study_cohort`, `audit_interaction_transcript`, `RemediationCatalog`, `evaluate_remediation_plan`, `evaluate_participant_sampling`, `AlphaSynthesisCatalog`, `synthesize_alpha_evidence`), re-exported cleanly via `crate::study` in root crate for 100% backward compatibility.
- [x] Extracted `crates/foi-gui` member crate under Cargo workspace containing pure M11 presentation-only HTML5/CSS/SVG generator, client state machine, viewport layout, accessibility tree, transport session protocol, asset governance manifests, browser interaction flow evaluation, and bidirectional presentation parity engine (`render_gui_html_document`, `verify_gui_html_document`, `assemble_gui_presentation_bundle`, `evaluate_presentation_need`, `verify_presentation_parity`, `audit_asset_governance`, `evaluate_browser_flow`, `GuiPresentationSession`, `GuiScenarioCatalog`, `GuiHtmlScenarioCatalog`, `GuiStateScenarioCatalog`, `AssetGovernanceCatalog`, `BrowserScenarioCatalog`), re-exported cleanly via `crate::gui` in root crate for 100% backward compatibility.
- [x] Extracted `crates/foi-alpha` member crate under Cargo workspace containing pure M12 public research-capable alpha release governance, backward compatibility policies, data dictionary fog-of-war redactions, research limitations/evidence boundaries, documentation guides audit, reproducibility package digests, and release readiness verification gates (`audit_release_archive_manifest`, `audit_release_checks`, `evaluate_compatibility_matrix`, `audit_data_dictionary`, `evaluate_alpha_governance`, `audit_guide_manifests`, `audit_limitations_and_boundaries`, `audit_reproducibility_bundle`, `AlphaScenarioCatalog`, `PublicAlphaGovernanceManifest`, `ReleaseChecksAuditReport`), re-exported cleanly via `crate::alpha` in root crate for 100% backward compatibility.
- 54 focused unit tests in `src/alpha/tests.rs`, `src/cli/tests.rs`, `src/bin/fog-of-intent-mcp.rs`, and `src/mcp/tests.rs` cover governance evaluation, fallback posture triggers, compliance basis points, compatibility matrix validation, migration contract requirements, data dictionary redaction auditing, limitations/evidence boundary auditing, guide section completeness, DAG cycle rejection, reproducibility checksum verification, release archive inventory verification, release readiness gate evaluation, critical blocker rejection, overclaim rejection, disclaimer enforcement, CLI scenario parsing, report formatting, MCP JSON-RPC tool/prompt/resource dispatch, fail-closed error handling, error Display coverage, and clean Markdown report hygiene.
