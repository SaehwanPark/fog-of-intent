...
impl LaneDelay {
  ...
  pub(crate) fn remaining_after(self, beats: u32) -> Option<Self> {
    let beats = u8::try_from(beats).ok()?;
    self
      .0
      .checked_sub(beats)
...
impl LaneDelayedEffects {
  ...
  pub fn push(&mut self, effect: LaneDelayedEffect) -> Result<(), LaneBoundsError> {
    ...
    if index < MAX_DELAYED_EFFECTS {
      self.items[index] = Some(effect);
      self.count += 1;
      Ok(())
    } else {
      Err(LaneBoundsError {
        value: self.count + 1,
        maximum: u8::try_from(MAX_DELAYED_EFFECTS)
          .expect("delayed effect capacity must fit in the bounded error value"),
      })
    }
  }
}
