...
fn observation_matches_command(
  state: &LaneSnapshot,
  receipt: &LaneObservationReceipt,
  command: &LaneIntentCommand,
) -> bool {
  let observation = receipt.observation;
  let observer_matches = observation.observer == command.actor;
  let observation_id_matches = observation.observation_id == command.observation_id;
  let schema_matches = observation.schema == M2_OBSERVATION_SCHEMA;
  let turn_matches = observation.turn == state.turn;
  let window_matches = observation.window == state.window;
  let source_hash_matches = receipt.source_state_hash == state.hash();
  observer_matches
...
pub fn validate_lane_command(
  ...
  if !observation_matches_command(state, receipt, command) {
