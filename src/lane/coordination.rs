...
pub fn resolve_coordination(
  ...
  let (disposition, support) = match response {
    ...
    ProposalResponse::Accept { .. } => match inputs.follow_through {
      FollowThrough::AllyCommitted => (CoordinationDisposition::AcceptedOffer, Some(offer.support)),
...
pub fn validate_coordinated_request(
  ...
  match request.response {
    ProposalResponse::Accept { .. } if request.intent.intent != offer.proposal.selected_intent => {
