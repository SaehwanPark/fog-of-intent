...
pub struct LaneExecutionInputs {
    ...
    pub(crate) flask_gained: LaneFlask,
    ...
    pub(crate) poultice_gained: LanePoultice,
    pub(crate) poultice_spent: LanePoultice,
...
impl LaneExecutionInputs {
    pub fn new(
        ...
        Self {
            ...
            poultice_gained: LanePoultice::zero(),
            poultice_spent: LanePoultice::zero(),
            delayed_effect: None,
        }
    }
    ...
    pub fn with_flask_gained(mut self, flask_gained: LaneFlask) -> Self {
        self.flask_gained = flask_gained;
        self
    }

    pub fn with_flask_spent(mut self, flask_spent: LaneFlask) -> Self {
        self.flask_spent = flask_spent;
        self
    }

    pub fn with_incense_gained(mut self, incense_gained: LaneIncense) -> Self {
        self.incense_gained = incense_gained;
        self
    }

    pub fn with_incense_spent(mut self, incense_spent: LaneIncense) -> Self {
        self.incense_spent = incense_spent;
        self
    }
    ...
    pub fn with_poultice_gained(mut self, poultice_gained: LanePoultice) -> Self {
        self.poultice_gained = poultice_gained;
        self
    }

    pub fn with_poultice_spent(mut self, poultice_spent: LanePoultice) -> Self {
        self.poultice_spent = poultice_spent;
        self
    }
    ...
    pub fn flask_gained(self) -> LaneFlask {
        self.flask_gained
    }

    pub fn flask_spent(self) -> LaneFlask {
        self.flask_spent
    }

    pub fn incense_gained(self) -> LaneIncense {
        self.incense_gained
    }
    ...
    pub fn poultice_gained(self) -> LanePoultice {
        self.poultice_gained
    }

    pub fn poultice_spent(self) -> LanePoultice {
        self.poultice_spent
    }
}
...
impl ResourceExecutionDeltas {
    fn from_execution(execution: LaneExecutionInputs) -> Self {
        Self {
            ...
            flask_gained: execution.flask_gained,
            ...
            poultice_gained: execution.poultice_gained,
            poultice_spent: execution.poultice_spent,
        }
    }
}
...
impl LaneEffect {
    pub fn provenance(self) -> LaneEffectProvenance {
        match self {
            ...
            | Self::PoulticeChanged { provenance, .. }
...
pub enum LaneExecutionError {
    ...
    PoulticeOverflow {
        gained: LanePoultice,
        current: LanePoultice,
    },
    InsufficientPoultice {
        spent: LanePoultice,
        available: LanePoultice,
    },
...
fn apply_player_resources(
    ...
    let after_spend_flask =
        before
            .flask
            .subtract(deltas.flask_spent)
            .ok_or(LaneExecutionError::InsufficientFlask {
                spent: deltas.flask_spent,
                available: before.flask,
            })?;
    let flask =
        after_spend_flask
            .add(deltas.flask_gained)
            .ok_or(LaneExecutionError::FlaskOverflow {
                gained: deltas.flask_gained,
                current: after_spend_flask,
            })?;
    let after_spend_incense = before.incense.subtract(deltas.incense_spent).ok_or(
        LaneExecutionError::InsufficientIncense {
            spent: deltas.incense_spent,
            available: before.incense,
        },
    )?;
    let incense = after_spend_incense.add(deltas.incense_gained).ok_or(
        LaneExecutionError::IncenseOverflow {
            gained: deltas.incense_gained,
    ...
    let after_spend_poultice = before.poultice.subtract(deltas.poultice_spent).ok_or(
        LaneExecutionError::InsufficientPoultice {
            spent: deltas.poultice_spent,
            available: before.poultice,
        },
    )?;
    let poultice = after_spend_poultice.add(deltas.poultice_gained).ok_or(
        LaneExecutionError::PoulticeOverflow {
            gained: deltas.poultice_gained,
            current: after_spend_poultice,
        },
    )?;
    Ok(PlayerResources {
        mana,
...
fn project_lane_events(
    ...
    if execution.flask_gained != LaneFlask::zero() {
        events.push(LaneEvent::FlaskGained {
            actor: player.id,
            amount: execution.flask_gained,
            trace,
        });
    }
    if execution.flask_spent != LaneFlask::zero() {
        events.push(LaneEvent::FlaskSpent {
            actor: player.id,
            amount: execution.flask_spent,
            trace,
        });
    }
    if execution.incense_gained != LaneIncense::zero() {
        events.push(LaneEvent::IncenseGained {
            actor: player.id,
            amount: execution.incense_gained,
            trace,
        });
    }
    if execution.incense_spent != LaneIncense::zero() {
        events.push(LaneEvent::IncenseSpent {
            actor: player.id,
            amount: execution.incense_spent,
            trace,
        });
    }
    ...
    if execution.poultice_gained != LanePoultice::zero() {
        events.push(LaneEvent::PoulticeGained {
            actor: player.id,
            amount: execution.poultice_gained,
            trace,
        });
    }
    if execution.poultice_spent != LanePoultice::zero() {
        events.push(LaneEvent::PoulticeSpent {
            actor: player.id,
            amount: execution.poultice_spent,
            trace,
        });
    }
...
fn project_lane_effects(
    ...
    if next_player.flask != player.flask {
        effects.push(LaneEffect::FlaskChanged {
            actor: player.id,
            before: player.flask,
            after: next_player.flask,
            cause: LaneEffectCause::Execution(trace),
            provenance: LaneEffectProvenance::direct_immediate(),
        });
    }
    if next_player.incense != player.incense {
        effects.push(LaneEffect::IncenseChanged {
            actor: player.id,
            before: player.incense,
            after: next_player.incense,
            cause: LaneEffectCause::Execution(trace),
            provenance: LaneEffectProvenance::direct_immediate(),
        });
    }
    ...
    if next_player.poultice != player.poultice {
        effects.push(LaneEffect::PoulticeChanged {
            actor: player.id,
            before: player.poultice,
            after: next_player.poultice,
            cause: LaneEffectCause::Execution(trace),
            provenance: LaneEffectProvenance::direct_immediate(),
        });
    }
