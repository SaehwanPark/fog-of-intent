...
`CliScenarioHost::actor_observation` now returns the current actor-visible
...
read-only and does not mutate host history or transition state.

## Verification

- One focused host regression proves exact mapper parity before and after the
  first fixture advance, actor-visible fields, hidden-field absence, and
  unchanged history.
